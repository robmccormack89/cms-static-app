...
class PaginationModel {
  ...
  public function __construct(int $count, $paged = true, $page = 1, $per_page = 7) {
    $this->count = $count;
    $this->paged = $paged;
    $this->page = $page;
  ...
  protected function setPaginationData() {
    ...
    // only do paged data stuff if paged is true & count is greater than 0. page & per_page are only required if paged is true
    if($this->paged == TRUE && $this->count > 0){
      
      // step 2 - if has next|prev, set the links data, available at .next & .prev
      if ($this->hasNextPage()) {
        $nexturl = $this->setPaginationLink(($this->page + 1));
        $data['next'] = $nexturl;
      };
  
      if ($this->hasPrevPage()) {
        $prevurl = $this->setPaginationLink(($this->page - 1));
        $data['prev'] = $prevurl;
      };
      
      // step 3 - all posts count divided by posts per page, rounded up to the highest integer
      $rounded = ceil($this->count / $this->per_page);
      ...
      $addifier = $page_offset * $this->per_page;
      $result_start = $addifier + 1;
      $result_end = $this->page * $this->per_page;
      ...
      if($this->per_page == 1) {
        $results_text = 'Showing Page '.$this->page.' of '.$this->count.' results';
      }
      
      if($this->per_page > 1 && $this->per_page < $this->count) {
    ...
    // else if paged is false but count still greater than zero
    elseif($this->paged == FALSE && $this->count > 0) {
      $results_text = 'Showing all '.$this->count.' results';
    }
    
    // else if count is less than 1
    elseif($this->count == 0) {
  ...
  protected function hasNextPage() {
    if($this->paged == false OR $this->page >= $this->count / $this->per_page) {
      return false;
    }
    return true;
  }  
  protected function hasPrevPage() {
    if($this->paged == false OR $this->page <= 1) {
      return false;
    }
    return true;
  }
